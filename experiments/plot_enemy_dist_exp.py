...
def plot_icorrelation(i_distance, i_saliency, individual=False):
    ...
    if individual:
        for i in range(len(i_distance)):
            ...
            # plt.legend()
            # plt.show()
            plt.title("Intervention on Enemy {}'s Location from Player".format(i+1))
        ...
        # plt.legend()
        # plt.show()
        plt.title("Intervention on Enemy Distance from Player", fontsize=16)
        plt.xlabel("Intervention on Distance to Player", fontsize=16)
        plt.ylabel("Difference in Saliency over Enemy", fontsize=16)
        plt.xticks([-4,-2,0,2,4])
        plt.axvline(x=0, color='black', linestyle='--', alpha=0.5)
        plt.plot(range(-4,5), [0]*len(range(-4,5)), '--', color='black', alpha=0.5)
        # plt.ylim(1,-1)
...
def plot(distances, saliency):
    ...
    plt.figure()
    for i in range(5):
        plt.plot(range(len(distances)), [t[i] for t in distances], label=labels[i], color=colors[i])
    ...
    plt.ylabel("Distance to Player", fontsize=16)
    plt.title("Distance of Enemies from Player Over Time", fontsize=16)
    # plt.legend()
...
def plot_correlation(distances, saliency, individual=False):
    ...
    if individual:
        for i in range(5):
            ...
            plt.title("Observed Distance of Enemy {} from Player".format(i+1))
            # plt.legend()
        ...
        for i in range(5):
            ...
            print(slope, intercept, r_value, p_value, std_err)
            plt.plot([t[i] for t in distances], intercept + np.multiply(slope,[t[i] for t in distances]), '-', color=colors[i])
        plt.ylabel("Saliency on Enemy", fontsize=16)
        plt.xlabel("Distance to Player", fontsize=16)
        plt.title("Observed Enemy Distance from Player", fontsize=16)
        plt.legend()
